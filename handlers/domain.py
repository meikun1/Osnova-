...
def _missing_config() -> list[str]:
    missing = []
    if not config.DOMAIN_CF_TOKEN:
        missing.append("DOMAIN_CF_TOKEN")
...
async def receive_domain(message: Message, state: FSMContext) -> None:
    ...
    try:
        ns_servers = await asyncio.to_thread(_bind_domain_sync, domain)
...
def _bind_domain_sync(domain: str) -> list[str]:
    """Синхронная проводка: CF zone + A + SSL + defaults + Caddy + reload."""
    token = config.DOMAIN_CF_TOKEN
    ip = config.DOMAIN_SERVER_IP
    caddyfile = config.DOMAIN_CADDYFILE
    caddy_exe = config.DOMAIN_CADDY_EXE
    target = config.DOMAIN_TARGET

    zone_id = cf_create_or_get_zone(domain, token)
    cf_add_a_record(zone_id, domain, ip, token)
    cf_set_ssl_mode(zone_id, "strict", token)
    cf_set_domain_defaults(zone_id, token)
    ns_servers = cf_get_ns_servers(zone_id, token)

    add_domain_to_caddy(domain, caddyfile, target=target)
    try:
        reload_caddy(caddy_exe, caddyfile)
    except CaddyReloadError:
        # откат — убираем блок из Caddyfile, чтобы не оставлять битый конфиг
        try:
