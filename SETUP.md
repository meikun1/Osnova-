...
уже заполнены). Останется вписать только **MANAGER_BOT_TOKEN** из @BotFather.
...
> Важно: с одним токеном должен работать **только один** инстанс бота —
> иначе два процесса будут конфликтовать на `getUpdates`.
...
## Перенос данных (по желанию)

Если нужно залить дамп существующей базы:
```bash
